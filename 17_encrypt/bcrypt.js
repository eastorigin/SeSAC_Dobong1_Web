...
// 1. 회원가입 >> 해시값 생성
function hashPW(pw) {
  // hashSync(비밀번호, 솔트라운드) // 리턴값은 암호화 된 문자(string)
  return bcrypt.hashSync(pw, saltRounds);
}

// 2. 로그인 >> 해시값 일치 확인
function comparePW(inputPw, hasedPw) {
  // compareSync(원본 비밀번호, 해시된 비밀번호)
  return bcrypt.compareSync(inputPw, hasedPw); // 리턴값은 t or f
}
...
// 비밀번호 일치 여부 확인
// 로그인 작업시 활용
const isMatch = comparePW("12345", hasedPw); // true
const isMatch2 = comparePW("1234", hasedPw); // false
